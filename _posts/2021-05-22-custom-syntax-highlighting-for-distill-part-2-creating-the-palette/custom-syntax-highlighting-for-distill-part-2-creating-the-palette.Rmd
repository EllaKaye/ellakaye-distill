...
In Part 1 of this series, [custom syntax highlighting for {distill}: modifying the theme **LINK**](), I describe why and how I went about the process of finding and modifying the default syntax highlighting for [{distill}]((https://rstudio.github.io/distill/)) and how I created a function that wrapped that process for [{distilltools}](https://ellakaye.github.io/distilltools). I also outline the four criteria that were important to me as I went about choosing colours for my own syntax highlighting theme:
...
The main pink that I use in this site has hex `r bg_col("#D4006A")`. I use a great little Mac app called [ColorSlurp](https://colorslurp.com) for building and assessing colour palettes. The basic version is free, though the pro version has great features for testing accessibility - more on below. In ColorSlurp, I started a new palette with #D4006A as a starting point, then, making sure I was in HSB mode, and found its HSB values are 330°, 100%, 83%. Leaving the S and B sliders where they are, the next colour I needed is 60° around the circle, i.e. at 30°, so I moved the H slider to that value and added the resulting orange to the palette. I then repeated that for 90°, 150°, 210° and 270°, giving me two shades of green, a blue and a purple. As I only need five colours, I discarded one of the greens, keeping the one at 150°, which is complementary to my main pink. 
...
My first thought was to keep the pink, blue and purple the same, and to just adjust the orange and green to a bit better than the necessary point. In the HSB space, that gives the palette in Figure \@ref(fig:orange-green-less-bright). 
...
My next thought was to reduce the brightness on all the colours to tje level of the least bright colour that passes the accessibility check (the green, at 53%). That gives the palette in Figure \@ref(fig:all-less-bright).
