...
- 8 are assigned a grey, [#5E5E5E]{.arrow-grey}, things like `Comment` and `Documentation`
- 1 is off-black, [#5E5E5E]{.arrow-black}, `Variable`
- 3 are blue, [#007BA5]{.arrow-blue}, `Other`, `ControlFlow` and `Keyword`
- 4 are green, [#20794D]{.arrow-green}, corresponding to various types of string
- 1 is purple, [#4758AB]{.arrow-purple}, `Function`
- 7 are red, [#AD0000]{.arrow-red}, a mix of numeric (e.g. `BaseN`, `Float`) and things like `Alert` and `Error`
- 1 is a brown, [#8F5902]{.arrow-brown}, for `Constant`
...
```{r hsl-colour-wheel, fig.cap="An HSL colour wheel. Screen shot from [https://www.canva.com/colors/color-wheel/](https://www.canva.com/colors/color-wheel/), annotations adapted from [https://dev.to/rneha725/hsl-and-hsla-in-css-31j3](https://dev.to/rneha725/hsl-and-hsla-in-css-31j3). ADD ALT TEXT.", echo = FALSE}
...
As noted at the beginning of this post, one key feature of the default syntax highlighting scheme for {distill} is that it is optimised for accessibility and colour contrast. According to the [Web Content Accessibility Guidelines](https://www.w3.org/WAI/standards-guidelines/wcag/), this requires that that contrast ratio between text and background colours be at least 4.5:1 for text of 14pt+ (which is the default text size for code in {distill}). 
